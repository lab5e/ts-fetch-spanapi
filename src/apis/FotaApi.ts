...
export class FotaApi extends runtime.BaseAPI {
    ...
    async createLabeledFirmwareRaw(requestParameters: CreateLabeledFirmwareRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<CreateLabeledFirmwareResponse>> {
        ...
        if (requestParameters.imageRefImageRef !== undefined) {
            queryParameters['imageRef.imageRef'] = requestParameters.imageRefImageRef;
        }

        if (requestParameters.imageRefCreatedAt !== undefined) {
            queryParameters['imageRef.createdAt'] = requestParameters.imageRefCreatedAt;
        }

        if (requestParameters.imageRefFileName !== undefined) {
            queryParameters['imageRef.fileName'] = requestParameters.imageRefFileName;
        }

        if (requestParameters.imageRefLength !== undefined) {
            queryParameters['imageRef.length'] = requestParameters.imageRefLength;
        }

        if (requestParameters.imageRefChecksum !== undefined) {
            queryParameters['imageRef.checksum'] = requestParameters.imageRefChecksum;
        }

        if (requestParameters.imageRefSha256 !== undefined) {
            queryParameters['imageRef.sha256'] = requestParameters.imageRefSha256;
        }
